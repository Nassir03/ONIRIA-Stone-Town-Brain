---
title: Room 15
type: room
status: draft
owner: Saleh Haji Othman
created: 2026-07-24
last_updated: 2026-07-24
confidence: medium
privacy: internal
task: 2
room_number: "15"
category: Supreme Room
floor: Second
source: ONIRIA Project Hub Reference
---

# Room 15

## Room Identity

Room 15 is a Supreme Room on the second floor.

## Category Features

Supreme Rooms include private in-room pool and ocean-view positioning, subject to final plan verification.

## Connected Records

- [[Second Floor]]
- [[Property Map]]
- [[Building Systems Register]]
- [[Room Inspection Structure]]
- [[Pool Safety Requirements]]
- [[Housekeeping SOPs]]
- [[Maintenance Request SOP]]
- [[Emergency SOPs]]

## Critical Systems

- Private pool services
- Waterproofing and leak detection
- HVAC and humidity management
- KNX lighting and comfort scenes
- Smart-lock access
- Wi-Fi, speakers, sensors

## Furniture and Equipment Summary

Complete after approved FF&E and source verification.

## Cleaning Standard

Use room readiness plus pool-related hygiene and safety checks.

## Maintenance Information

Guest-impact defaults to high for water, power, AC, and pool defects.
