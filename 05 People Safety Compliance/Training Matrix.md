---
title: "Training Matrix"
type: training-matrix
status: active
owner: "ONIRIA Operations"
created: 2026-07-24
last_updated: 2026-07-24
confidence: medium
privacy: internal
---

# Training Matrix

| Training | All staff | Managers | Front/Guest | Housekeeping | Engineering | Spa/F&B | Security | Refresh trigger |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| ONIRIA service standards | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Annual/change |
| Guest data privacy | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Annual/incident |
| Emergency awareness | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Drill/change |
| Fire role training | Role-based | ✓ | Role-based | Role-based | Role-based | Role-based | ✓ | Approved plan |
| First aid | Selected | Selected | Selected | Selected | Selected | Selected | Selected | Certificate |
| Food allergy awareness | Awareness | ✓ | ✓ | Awareness |  | ✓ |  | Annual/incident |
| Pool safety | Awareness | ✓ | Awareness |  | Engineering | Relevant | ✓ | Seasonal/incident |
| Room access/security | ✓ | ✓ | ✓ | ✓ | ✓ | Relevant | ✓ | Annual/incident |
| PMS/task/communication systems | Role-based | ✓ | ✓ | Role-based | ✓ | Role-based | ✓ | System change |
| Asset safety/isolation |  | Relevant |  | Awareness | ✓ | Relevant | ✓ | Equipment/change |

Final required certificates, frequencies and competent providers are TO CONFIRM against approved requirements.
